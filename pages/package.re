...
let wrap = (v) => "<span style='color: #333; font-size: 0.75em'>{ " ++ v ++ " }</span>" 
    ...
    switch (conflicts) {
      ...
      | lst => {
        <div>
          ...
          {conflicts |> List.map(((name, constr)) => {
            OpamPackage.Name.to_string(name) ++ " " ++ (Option.map(wrap, constr) |> Option.value(~default=""))
          }) |>String.concat("<br/>") |> Html.Unsafe.data}
          </div>
        </div>
      };
    }
...
module Header = {
  ...
  let all_package_versions_select =
    ...
    let tabs = switch((mode, docs_status)) {
      | (Info, Documentation.Package.Built(_)) => 
        [<div style="margin-right: 2rem; text-decoration: underline;">"Info"</div>,
        <a href=docs_url>"Docs"</a>]
      | (Info, status) => 
        [<div style="margin-right: 2rem; text-decoration: underline;">"Info"</div>,
         <div style="color: gray">"Docs"<span className="caption">{"⚠️ " ++ docs_status_message |> Html.txt}</span></div>]
      | (Docs, _) => 
        [ <a href=info_url style="margin-right: 2rem;">"Info"</a>,
          <a href=docs_url style="text-decoration: underline">"Docs"</a>]
    };

    let path_items = String.split_on_char('/', path) 
      |> List.filter((item) => switch(item) {
        | "" | "index.html" => false
        | _ => true
      })
    ...
    let (_, path_indicator) = 
      List.fold_left_map(((i, prefix), item) => 
      {
        ...
        if (i == l-1) {
          ...
          (next, <span>" » "<a href={link ++ "/"}>{item |> Html.txt}</a></span>)
        }}, (0, info_url), path_items);

    <div style="border-bottom: solid #0002 1px; margin-bottom: 1rem; display: flex; align-items: baseline;">
      <h1 style="display: inline; border-bottom: solid 0.5rem orange; margin-bottom: -0.5rem;">{name |> Html.txt}</h1>
...
let render = (~state, ~name, ~version, ~info: Docs2web.Package.Info.t, ~docs, ~docs_status, ~all_versions, ~path) => {
  ...
  if (path == "") {
    ...
    <Header 
      ...
      mode={Info} />
    <br/>
    <div style="border-bottom: solid #0002 1px; padding-bottom: 1rem; margin-bottom: 1rem; ">
      <h2>{info.synopsis |> Html.txt}</h2>
      <p>
        {info.description |> Omd.of_string |> Omd.to_html |> Html.Unsafe.data}
      </p>
    </div>
    <div>
      <h2>"Package information"</h2>
      <div style="display: flex">
        <div style="flex: 1; border-right: solid #0002 1px; padding-bottom: 1rem; margin-bottom: 1rem; margin-right: 1rem; padding-right: 1rem;  ">
          ...
          <div>
            <h3>"Homepage "</h3>
            <div style="padding-left: 1rem">
            ...{info.homepage |> List.map((addr) => <a href=addr>{addr |> Html.txt}</a>) }
            </div>
          </div>
          <div>
            <h3>"Maintainer "</h3>
...
let v = (~state, ~kind, ~name, ~version, ~path, ()) => {
  ...
  let docs_info = Documentation.package_info(docs, opam);

  let docs_status = switch(docs_info) {
    | None => Documentation.Package.Unknown
    | Some (t) => Documentation.Package.status(t) 
  };

  let docs_target = prefix(kind) ++ "/" ++ OpamPackage.Name.to_string(name) ++ "/" ++ OpamPackage.Version.to_string(version) ++ "/" ++ path;

  let+ docs = switch(docs_status) {
    | Built(_) => Docs2web.Documentation.load(State.docs(state),docs_target)
    | Pending => Lwt.return_error (`Msg ("Documentation is currently building."))
    | Failed => Lwt.return_error (`Msg ("Documentation failed to build."))
    | Unknown => Lwt.return_error (`Msg ("The docs CI don't know yet about this package."))
  };
