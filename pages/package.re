...
let render = (~state, ~name, ~version, ~info, ~docs=?, ~all_versions, ~path) => {
  ...
  switch (docs) {
  | None =>
    <Template header title> <br /> <div> "No documentation" </div> </Template>
  | Some(docs) => <Template header title> <br /> docs </Template>
  };
};
